...
func newDirectUploads(defaultClient, securityClient HTTPClient, serverURL, language, sdkVersion, genVersion string) *directUploads {
...
func (s *directUploads) ListDirectUploads(ctx context.Context, request operations.ListDirectUploadsRequest, opts ...operations.Option) (*operations.ListDirectUploadsResponse, error) {
	...
	case httpRes.StatusCode == 200:
		switch {
		case utils.MatchContentType(contentType, `application/json`):
			var out *shared.ListUploadsResponse
			if err := utils.UnmarshalJsonFromResponseBody(httpRes.Body, &out); err != nil {
				return nil, err
			}

			res.ListUploadsResponse = out
		}
	}
